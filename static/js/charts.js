...
function buildCharts(sample) {
  ...
  d3.json("samples.json").then((data) => {
    ...
    var sampleFilter = samplesArray.filter(sampleObject => sampleObject.id == sample);
    ...
    var barLayout = {
      title: { 
       text: "The Top 10 Bacteria Cultures Found", font: { size: 18 }
       },
       height:300,
       width:350,
...
// Bar and Bubble charts
    ...
    var bubbleLayout = {
      title: {
        text: "Bacteria Cultures Per Sample", font: { size: 20 }
      },
      xaxis: {title: "OTU ID"},
      hovermode: 'closest',
      showlegend: false,
  ...
  Plotly.newPlot("bubble", bubbleData, bubbleLayout); 
    ...
    var metaFilter = data.metadata
    var metaArray = metaFilter.filter(sampleObject => sampleObject.id == sample);
    // Create a variable that holds the first sample in the array.
          // //  5. Create a variable that holds the first sample in the array.
          // var sampleFirst = sampleFilter[0];

    // 2. Create a variable that holds the first sample in the metadata array.
    var metaSampleFirst = metaArray[0];
    ...
    var washnFreqcy = parseFloat(metaSampleFirst.wfreq);
